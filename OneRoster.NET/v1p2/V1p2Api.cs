...
namespace OneRoster.NET.v1p2
{
    public class V1p2Api : IDisposable
    {
        ...
        public V1p2Api(string tokenUrl, string baseUrl, string clientId, string clientSecret)
        {
            _token = new Oauth2(tokenUrl).GetToken(clientId, clientSecret);
        ...
        internal IRestResponse GetResponse(RestRequest request)
        {
            request.AddHeader("Authorization", "Bearer " + _token.access_token);
